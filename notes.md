...
```ruby
filepath = Protocol::URL[event.request.path].path.local_path
```

This call is security-sensitive because it converts an untrusted request path into a filesystem path. Confirm that the updated code intentionally rejects decoded components containing filesystem separators before merging the result with `web_root`.
